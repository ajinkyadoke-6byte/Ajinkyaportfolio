...
const Skills = () => {
  return (
    <motion.div
      ...
      viewport={{ once: false, amount: 0.2 }}
      id="skills"
      className="py-14 sm:py-16 md:py-20 bg-dark-100 scroll-mt-24"
    >
      <div className="container mx-auto px-4 sm:px-6">
        <h2 className="text-2xl sm:text-3xl font-bold text-center mb-6 sm:mb-10">
          My <span className="text-purple">Skills</span>
        </h2>
        <p className="text-gray-400 text-sm sm:text-base text-center max-w-2xl mx-auto mb-8 sm:mb-12">
          Technologies I work with to bring ideas to life
        </p>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 sm:gap-8 lg:gap-10 max-w-5xl mx-auto">
          {skillsData.map((skill, index) => {
            const Icon = skill.icon
            return (
              <motion.div
                ...
                transition={{ duration: 0.6, delay: index * 0.1, ease: 'easeOut' }}
                viewport={{ once: false, amount: 0.2 }}
                className="bg-dark-200 rounded-lg p-4 sm:p-5 hover:-translate-y-1 transition-transform duration-300"
              >
                <Icon className="text-purple mb-2" size={22} />
                <h3 className="text-sm sm:text-base font-semibold mb-1.5">{skill.title}</h3>
                <p className="text-gray-400 text-xs sm:text-sm mb-3 leading-snug">{skill.description}</p>
                <div className="flex flex-wrap gap-1.5">
                  {skill.tags.map((tag) => (
                    <span
                      key={tag}
                      className="px-2 py-0.5 text-[10px] sm:text-xs rounded-full bg-dark-100 text-gray-300"
                    >
                      {tag}
                    </span>
                  ))}
